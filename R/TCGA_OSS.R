OSS <- function(data = data, gene = gene, cut.point = "0.5", out.format = "r"){
  ...
  p <- (surv + N_T + stage)/ surv_stage / (surv_stage2 + stage2)

  if(!dir.exists("batchplots")) {dir.create("batchplots")}

  if(out.format == "jpg") {ggsave(paste0(gene," plot.jpg"), p, path = "batchplots",dpi = 600, width = 14, height = 12)
  } else if (out.format == "pptx") {
    export::graph2ppt(p, paste0("batchplots/",gene," plot in TCGA_KIRC.pptx"), width = 14, height = 12)
  }else if (out.format == "pdf") {
    ggsave(paste0(gene," plot.pdf"), p, path = "batchplots",dpi = 600, width = 14, height = 12)
